; ============================================================================
; CLEO - game logic, ported from the Master build's src/logic.s (which is itself
; the port of CleoApp.run()).  This is a COPY, not an include: the Master engine is
; 65C02 and its zero page is its own, and beeb/src is kept free of MODELB
; conditionals.  tools/logicdiff.sh reports what has drifted between the two.
;
; What is different here, and only this:
;   - the 65C02 instructions are the macros in cpu.inc (stz, bra, phx, ...)
;   - the logic's persistent variables live in bank 7 (LGCDATA), not zero page:
;     ca65 assembles `lda px' as absolute without being told, and only ptr, t16
;     and w16 are ever used as (zp),y
;   - its transient temps overlay the renderer's scratch, which is dead while the
;     logic runs (LTMP in zp.inc)
;   - the window is 160x84 game pixels here, not 160x120
; ============================================================================
; ---------------------------------------------------------------- 16-bit macros
.macro mov16 dst, src
        lda src
...
; OBJN is in defs.inc here: this target carries one level of 24 objects, not 149
O_STAMP = LV_OBJST
O_TYPE  = O_STAMP + OBJN
O_XL    = O_TYPE + OBJN
O_XH    = O_XL + OBJN
...
; ---- the logic's variables ------------------------------------------------------
; The persistent ones live in bank 7 beside the code that reads them: ca65 assembles
; `lda px' as absolute without being told, and nothing here is ever used as (zp),y.
        .segment "LGCBSS"
frame:    .res 2
...
level_init:
        ...
        lda #0
:       sta O_STAMP,x
        sta O_STAMP+OBJN*4,x
        sta O_STAMP+OBJN*8,x
        sta O_STAMP+OBJN*12,x
        inx
        cpx #OBJN*4
        bne :-
        ldx #127
...
@count: inc fe
        lda fe
        and #3                      ; no bit #imm on a 6502; fe is reloaded below
        bne @done
        lda fe
        cmp #12
        bcs :+
...
; ============================================================================
        .segment "LGCCODE"          ; (the Master keeps this in main RAM to page the
                                    ;  HUD bank; here the digits are in this bank)
