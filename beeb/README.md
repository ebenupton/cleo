...
* **Display**: MODE 2, each square game pixel becomes two vertically adjacent MODE 2
  pixels (1 MODE 2 pixel wide, 2 scanlines tall). Colours are approximated with a 4x4
  ordered dither over the 8-colour palette (`tools/convert.py`).
...
* **Logic**: a direct port of `CleoApp.run()` (all 13 object types, the player physics
  ...
  the 2 MHz 6502 manages (about 16-25 fps). Sprites that did not change since the
