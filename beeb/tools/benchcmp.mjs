...
for (let r = 1; r < runs.length; r++) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const k of ["jumpI", "runI", "jumpL", "runL", "jumpLI", "runLI"])
      if (a[k] != null && b[k] != null) (ds[k] ??= []).push(b[k] - a[k]);
  ...
  if (ds.jumpI?.length) console.log(`  RENDER instructions: d(jump) median ${med(ds.jumpI)};  d(run) median ${med(ds.runI)}`);
  if (ds.jumpL?.length) console.log(`  LOGIC  cycles: d(jump) median ${med(ds.jumpL)};  d(run) median ${med(ds.runL)}   instructions: ${med(ds.jumpLI)} / ${med(ds.runLI)}`);
}
if (sceneOk) {
