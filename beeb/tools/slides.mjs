...
// screen file (20480 bytes for $3000) a Model B can *LOAD in MODE 1.
//   node tools/slides.mjs <disc> <labels> <level> <frames> <out>
// The framebuffer is read at frame_top like shot.mjs; the picture (bar + 30 rows) is the
// 256 scanlines from 32, i.e. the whole of a standard MODE 1 screen.
...
console.log(`wrote ${out}: level ${lvS} frame ${N}, cleo at (${H.rd(H.A.px)|(H.rd(H.A.px+1)<<8)},${H.rd(H.A.py)|(H.rd(H.A.py+1)<<8)})`);
process.exit(0);
