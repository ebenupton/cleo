...
if MODE == 2:
    ...
    # wanted YWMWWYWM (scan order over the 2x4 kernel) is not any colour's natural
    # dither -- its magentas sit on the kernel's first-lighting cells -- so it is laid by
    # hand, in the same tile-local phase as the Bayer kernel.
    _C1 = {'Y': 3, 'W': 7, 'M': 5}
    TIL_PATTERN[(221, 187, 119)] = lambda x, line: _C1['YWMWWYWM'[(line & 3) * 2 + (x & 1)]]
