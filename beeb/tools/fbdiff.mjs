// Capture the rendered window (back buffer at render_done) per logic frame for a scripted run,
// to compare two builds pixel-exactly.   node tools/fbdiff.mjs <disc> <labels.txt> <out.json> [level]
...
wr(A.title_loop, 0xa9); wr(A.title_loop + 1, 0x00); wr(A.title_loop + 2, 0xea); wr(A.level_loop + 3, 0xa2); wr(A.level_loop + 4, lvl);
...
cpu.debugInstruction.add((pc) => {
    if (pc === A.game_frame) {
        ...
        // rows 0..26 of the window plus the A row (lines wfine..7) and row 27 (lines 0..wfine-1)
        const S = (rd(A.ringS) | (rd(A.ringS + 1) << 8)) * 8, wf = rd(A.wfine);
...
writeFileSync(out, JSON.stringify({ frames, renders, windows }));
